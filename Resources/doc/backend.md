...
public function previewEnumExtraState(array $enum): array 
{
    ...
    return array_merget($enum, $extraEnum);
}
```
...
> 对字段的渲染处理，类似 `@Preview` 中的 `dress` 和 `render` 参数。
...
- **数据处理函数**
    ...
    > 当你的数据提交后不只是对单表进行操作时，框架就没办法帮你自动完成持久化了，你需要手动将提交的数据进行自定义持久化，此时你就可以手动管理。
